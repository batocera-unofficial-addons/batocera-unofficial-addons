...
PORT_SCRIPT="/userdata/roms/ports/HBOMax.sh"
ICON_PATH="/userdata/roms/ports/images/hbo-logo.jpg"
KEYS_PATH="/userdata/roms/ports/HBOMax.sh.keys"
LOGO_URL="https://github.com/batocera-unofficial-addons/batocera-unofficial-addons/raw/main/hbomax/extra/hbo-logo.jpg"
...
cat << EOF > $PORT_SCRIPT
#!/bin/bash
DISPLAY=:0.0 $APPPATH --no-sandbox --test-type --start-fullscreen --force-device-scale-factor=1.6 'play.hbomax.com'
EOF

chmod +x $PORT_SCRIPT

# Step 4: Download the icon
echo "Downloading HBO Max logo..."
...
# Step 6: Add HBO Max entry to gamelist.xml
echo "Updating gamelist.xml..."
xmlstarlet ed -s "/gameList" -t elem -n "game" -v "" \
  -s "/gameList/game[last()]" -t elem -n "path" -v "./HBOMax.sh" \
  -s "/gameList/game[last()]" -t elem -n "name" -v "$APPNAME" \
  -s "/gameList/game[last()]" -t elem -n "image" -v "./images/hbo-logo.jpg" \
...
echo "$APPNAME port setup completed. You can now access HBO Max through Ports!"
