...
PORT_SCRIPT="/userdata/roms/ports/Mubi.sh"
ICON_PATH="/userdata/roms/ports/images/mubi-logo.jpg"
KEYS_PATH="/userdata/roms/ports/Mubi.sh.keys"
LOGO_URL="https://github.com/batocera-unofficial-addons/batocera-unofficial-addons/raw/main/mubi/extra/mubi-logo.jpg"
...
cat << EOF > $PORT_SCRIPT
#!/bin/bash
DISPLAY=:0.0 $APPPATH --no-sandbox --test-type --start-fullscreen --force-device-scale-factor=1.6 'mubi.com/'
EOF

chmod +x $PORT_SCRIPT

# Step 4: Download the icon
echo "Downloading Mubi logo..."
...
xmlstarlet ed -s "/gameList" -t elem -n "game" -v "" \
  -s "/gameList/game[last()]" -t elem -n "path" -v "./Mubi.sh" \
  -s "/gameList/game[last()]" -t elem -n "name" -v "$APPNAME" \
  -s "/gameList/game[last()]" -t elem -n "image" -v "./images/mubi-logo.jpg" \
...
echo "$APPNAME port setup completed. You can now access Mubi through Ports!"
