...
# Encrypted password and Base64-encoded password key
...
# Decrypt the password key and password
decrypt_password() {
    local key=$(echo "$password_key_base64" | base64 -d)
...
show_menu() {
    ...
    if [[ -z "$correct_password" ]]; then
        echo "Error: Unable to decrypt the password." >&2
        exit 1
    fi
