...
PORT_SCRIPT="/userdata/roms/ports/DisneyPlus.sh"
ICON_PATH="/userdata/roms/ports/images/disneyplus-logo.png"
KEYS_PATH="/userdata/roms/ports/DisneyPlus.sh.keys"
LOGO_URL="https://github.com/batocera-unofficial-addons/batocera-unofficial-addons/raw/main/disneyplus/extra/disneyplus-logo.png"
...
cat << EOF > $PORT_SCRIPT
#!/bin/bash
DISPLAY=:0.0 $APPPATH --no-sandbox --test-type --start-fullscreen --force-device-scale-factor=1.6 'disneyplus.com'
EOF

chmod +x $PORT_SCRIPT

# Step 4: Download the icon
echo "Downloading $APPNAME logo..."
...
# Step 6: Add $APPNAME entry to gamelist.xml
echo "Updating gamelist.xml..."
xmlstarlet ed -s "/gameList" -t elem -n "game" -v "" \
  -s "/gameList/game[last()]" -t elem -n "path" -v "./DisneyPlus.sh" \
  -s "/gameList/game[last()]" -t elem -n "name" -v "$APPNAME" \
  -s "/gameList/game[last()]" -t elem -n "image" -v "./images/disneyplus-logo.png" \
