...
if [ "$arch" == "x86_64" ]; then
    ...
    appimage_url=$(curl -s https://api.github.com/repos/ivan-hc/Chrome-appimage/releases/latest | jq -r '.assets[] | select(.name? and (.name | test("stable.*\\.AppImage$"))) | .browser_download_url' | head -n 1)
else
